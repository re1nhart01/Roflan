...
export const getTimeStampForRefreshToken = (timeBetween: number) => {
  const now = Date.now();
  const toMilliseconds = timeBetween * 1000;

  // padding to save that could be some edge case like long time to setup into storage.
  const padding = 2000000;
  return now + toMilliseconds - padding;
};
